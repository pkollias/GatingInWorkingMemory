...
def beh_unit_fr_plot(u_iloc, version_fr, version_condition, legend_show=True):
    ...
    fig, ax = plot_unit(unit_ind, pbt_mean, pbt_se, version_condition, legend_show)
    return fig, ax
...
def get_unit_linepoints(timebin_fr_dict, unit_ind, timebin_interval, condition_columns, condition_series):
    ...
    pbt_smooth.crop_timeseries(-50 - timebin_interval.timebin, 1000)
...
def get_condition_plot_params(condition, version_condition):
    ...
    # if version_condition == 'StimulusPreDistGating':
...
def plot_unit(unit_ind, pbt_mean, pbt_se, version_condition, legend_show=True):
