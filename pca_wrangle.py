...
def main():
    ...
    for ind, unit_index in enumerate(units.index):
        ...
        if bool(timebin_fr_dict):
            ...
            # create column for pca condition values
            events_conditions_slice['PCA_Condition'] = events_conditions_slice.apply(lambda row: tuple([row[col] for col in condition_columns]), axis=1)

            # filter only rows that belong in condition list
            events_conditions_collection = events_conditions_slice[events_conditions_slice['PCA_Condition'].isin(condition_list)]
        ...
        pca_condition_events_dict[unit_index] = unit_dict

    md.np_saver(pca_condition_events_dict, target_filename)
